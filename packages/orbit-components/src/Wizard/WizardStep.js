...
const StyledProgressBar = styled.div`
  ...
  &:after {
    ...
    display: block;
    position: absolute;
    top: ${({ iconHeight }) => parseFloat(iconHeight) / 2 - 1}px;
    width: 50%;
    height: 2px;
  }
  &:before {
  ...
  &:after {
    ${right}: 0;
    background: ${({ theme, status, nextStepStatus }) =>
...
const WizardStep = ({ dataTest, title, onClick }: Props) => {
  ...
  if (!isTablet) {
    return (
      <StyledContainer data-test={dataTest} isTablet={isTablet} status={status}>
